...
export function BillProcessing() {
  const { friends, creditCards, addExpense, updateCreditCard } = useExpense();
        ...
        bill.id === billId
          ? {
              ...
              transactions: bill.transactions.map(t =>
                t.id === transactionId
                  ? { ...t, status: 'assigned', assignedFriendId: friendId }
                  : t
              )
            }
          : bill
  ...
  const handleProcessBill = (billId: string) => {
    ...
    const assignedTransactions = bill.transactions.filter(
      t => t.status === 'assigned'
    );

    // Create expenses for assigned transactions using context
        ...
        b.id === billId
          ? {
              ...
              status:
                ...
                b.transactions.filter(t => t.status !== 'ignored').length
                  ? 'processed'
                  : 'partially_processed'
            }
          : b
    ...
    // Update credit card balance using context
    const card = creditCards.find(c => c.id === bill.cardId);
    if (card) {
      updateCreditCard(card.id, {
        currentBalance: card.currentBalance + bill.totalAmount
      });
    }
  };
    ...
    const bill: ProcessedBill = {
      ...
      dueDate:
        ...
        new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
          ...
          .split('T')[0],
      totalAmount:
        manualBill.totalAmount ||
        manualBill.transactions.reduce((sum, t) => sum + t.amount, 0),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Paste your credit card bill text or statement details to
          ...
          {/* ... existing code for manual entry dialog ... */}
          <Dialog open={isManualEntryOpen} onOpenChange={setIsManualEntryOpen}>
            ...
            <DialogContent className="max-w-4xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Manually create a bill with individual transactions.
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={value =>
                        setManualBill({ ...manualBill, cardId: value })
                      }>
                      <SelectTrigger>
                        <SelectValue placeholder="Select credit card" />
                      </SelectTrigger>
                  ...
                  {manualBill.transactions.length > 0 && (
                    <div className="space-y-2">
                      <h5 className="font-medium">
                        Transactions ({manualBill.transactions.length})
                      </h5>
                          ...
                          <div className="text-right">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                setManualBill({
                                  ...manualBill,
                                  transactions: manualBill.transactions.filter(
      ...
      <div className="space-y-4">
        {processedBills.length > 0 ? (
          processedBills.map(bill => {
            const card = creditCards.find(c => c.id === bill.cardId);
            const unassignedCount = bill.transactions.filter(
              t => t.status === 'unassigned'
            ).length;
            const assignedCount = bill.transactions.filter(
              t => t.status === 'assigned'
            ).length;
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          {transaction.status === 'assigned' && (
                            <Badge variant="default">
                              {
                                friends.find(
                                  f => f.id === transaction.assignedFriendId
                                )?.name
                              }
                            </Badge>
                          )}
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground text-center">
                No bills processed yet. Upload a credit card statement or create
                a manual bill to get started.
              </p>
            </CardContent>
          </Card>
        )}
